sap.ui.define([
	...
	return BaseObject.extend("CTT.Reabastecimento.model.GroupSortState", {
		 ...
		 * @alias CTT.Reabastecimento.model.GroupSortState
		 */
		...
		/**
		 * Groups by Quantity, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Quantity") {
				...
				this._oViewModel.setProperty("/sortBy", "Quantity");

				aSorters.push(
					new Sorter("Quantity", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "UC");
			}
